...
function ready(_ref) {
  var _ref2 = _slicedToArray(_ref, 3),
      json = _ref2[0],
      datapoints = _ref2[1],
      coordinateData = _ref2[2];

  console.log('What is our data?'); // console.log(json)
  // console.log(countries)

  coordinateData.forEach(function (d) {
  ...
  svg.append('path').datum({
    type: 'Sphere'
  }).attr('d', path).attr('stroke-width', 1).attr('fill', 'lightblue');
  svg.selectAll('path').data(countries.features).enter().append('path').attr('class', 'country').attr('d', path).attr('fill', 'grey');
  svg.selectAll('.city').data(coordinateData).enter().append('circle').attr('class', 'city').attr('r', 2).attr('fill', 'white').attr('transform', function (d) {
    var coords = [d.longitude, d.latitude];
    return "translate(".concat(projection(coords), ")");
  });
  svg.selectAll('.transit').data(coordinateData).enter().append('path').attr('d', function (d) {
    var fromCoords = [-74, 40];
    var toCoords = coordinateStore.get(d.iata_code);
    var geoLine = {
      type: 'LineString',
      coordinates: [fromCoords, toCoords]
    };
    return path(geoLine);
  }).attr('fill', 'none').attr('stroke', 'white').attr('stroke-width', 1).attr('stroke-linecap', 'round');
}
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","/data/world.topojson":"data/world.topojson","/data/flights.csv":"data/flights.csv","/data/airport-codes-subset.csv":"data/airport-codes-subset.csv"}]},{},["scripts/homework-17/02-map.js"], null)
