...
var svg = d3.select('#chart-1').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).style('background', 'black').append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').style('background', 'black');
...
function ready(_ref) {
  ...
  var countries = topojson.feature(json, json.objects.countries); // console.log(json)
  // console.log(countries)

  var population = d3.extent(datapoints, function (d) {
    return d.population;
  });
  var colorScale = d3.scaleSequential(d3.interpolateCool).domain(population).clamp(true);
  console.log(graticule());
  svg.append('path').datum(graticule()).attr('d', path).attr('stroke', 'lightgrey').attr('fill', 'none');
  svg.selectAll('path').data(countries.features).enter().append('path').attr('class', 'country').attr('d', path).attr('fill', 'black');
  svg.selectAll('.cities').data(datapoints).enter().append('circle').attr('class', 'cities').attr('r', .5).attr('transform', function (d) {
