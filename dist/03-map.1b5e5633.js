...
var colorScale = d3.scaleLinear().range(['pink', 'lightblue', 'purple']);
Promise.all([d3.xml(require('/data/canada-2.svg')), d3.csv(require('/data/wolves.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  ...
  var wolves = datapoints.map(function (d) {
    return +d.wolves;
  });
  colorScale.domain(d3.extent(wolves)); //Get ready to process the hexagon svg file with D3

  var imported = d3.select(hexFile).select('svg'); // Remove the stylesheets Illustrator saved

  imported.selectAll('style').remove(); // Inject the imported svg's contents into our real svg
  ...
  svg.selectAll('.hex-group').data(datapoints).enter().append('text').attr('class', 'state-label').text(function (d) {
    return d.abbreviation;
  }).style('text-shadow', 'black 1px 1px 1px');
}
},{"d3":"../node_modules/d3/index.js","/data/canada-2.svg":"data/canada-2.svg","/data/wolves.csv":"data/wolves.csv"}]},{},["scripts/homework-17/03-map.js"], null)
